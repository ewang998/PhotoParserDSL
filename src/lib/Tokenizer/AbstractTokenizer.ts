export abstract class AbstractTokenizer {
    ...
    public checkNext(regexp: RegExp) {
        console.log(`testing ${this.tokens[this.currentToken]} with ${regexp}`);
    ...
    public getAndCheckNextSequence(regexps: RegExp[]): string[] | never {
        ...
        for (let regexp of regexps) {
            try {
                ...
                throw new Error(
                    `Unexpected token. Expected a match for ${regexps.toString()}, but recieved ${
                        this.tokens[this.currentToken]
                    }. Successfully matched ${results.toString()}`
                );
            }
        }
